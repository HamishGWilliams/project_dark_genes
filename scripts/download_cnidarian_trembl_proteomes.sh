...
# selected trembl cnidarian proteomes only

# capture UPIDs to download TrEMBL proteomes
awk 'NR>1 {print $1}' \
  02_annotation/reference_dbs/cnidaria_proteomes.tsv \
  > 02_annotation/reference_dbs/cnidaria_selected_upids.txt

# Download
OUT=02_annotation/reference_dbs/trembl_cnidaria_selected.fasta
: > "$OUT"
...
while read -r UPID; do
  ...
  curl -L -G 'https://rest.uniprot.org/uniprotkb/stream' \
    ...
    >> "$OUT"
  echo >> "$OUT"
done < 02_annotation/reference_dbs/cnidaria_selected_upids.txt
